...
namespace VantagePoint
{
    public partial class Startup
    {
        ...
        {
            SqlErrorStore store = null;

            Action<ExceptionalSettings> configure = settings =>
            {
                settings.Store.ApplicationName = "VantagePoint";
                settings.Store.ConnectionString = configuration.GetValue<string>("Data:Default:ConnectionString");
                settings.DefaultStore = store ?? (store = new SqlErrorStore(settings.Store, configuration.GetValue<string>("Data:Default:ProviderName")));
            ...
            Exceptional.Configure(configure);
            services.AddExceptional(configure);
