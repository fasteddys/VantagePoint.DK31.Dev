...
using System.Text;
using VantagePoint.Administration.Repositories;
using VantagePoint.Common;

namespace VantagePoint.Membership.Pages
{
                 ...
                 (user.TwoFactorAuth == Administration.TwoFactorAuthType.Email)))
            {
                ...
                if (user.TwoFactorAuth == Administration.TwoFactorAuthType.SMS)
                {
                    ...
                    Dependency.Resolve<Administration.ISMSService>().Send(
                        phoneNumber: mobile,
                        text: "Please use code " + data.TwoFactorCode + " for VantagePoint login.",
                        reason: "Sent by VantagePoint system for two factor authenication by SMS (" + user.Username + ")");
                ...
                else
                {
                    EmailHelper.Send(
                        address: user.Email,
                        subject: "Your two-factor authentication code for VantagePoint login",
                        body: "Please use code " + data.TwoFactorCode + " for VantagePoint login.");
