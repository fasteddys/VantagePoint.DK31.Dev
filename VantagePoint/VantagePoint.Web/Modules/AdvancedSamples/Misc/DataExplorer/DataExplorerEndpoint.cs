...
{
    [Route("Services/AdvancedSamples/DataExplorer/[action]")]
    [ServiceAuthorize(Administration.PermissionKeys.Security)]
    public class DataExplorerController : ServiceEndpoint
    {
        public ListResponse<DataExplorerConnection> ListConnections(ListRequest request)
        {
            request.CheckNotNull();

            var response = new ListResponse<DataExplorerConnection>();
            response.Entities = new List<DataExplorerConnection>();

            var config = Config.Get<DataExplorerConfig>();

            foreach (var connectionKey in new string[] { "Default", "Northwind" })
            {
                if ((config.IncludeConnections == null ||
                     config.IncludeConnections.Contains(connectionKey, StringComparer.OrdinalIgnoreCase)) &&
                    (config.ExcludeConnections == null ||
                     !config.ExcludeConnections.Contains(connectionKey, StringComparer.OrdinalIgnoreCase)))
                {
                    response.Entities.Add(new DataExplorerConnection
                    {
                        Key = connectionKey
                    });
                }
            ...
            IEnumerable<DataExplorerConnection> result = response.Entities;
        ...
        public ListResponse<DataExplorerTable> ListTables(DataExplorerListTablesRequest request)
        {
            request.CheckNotNull();
            ...
            var response = new ListResponse<DataExplorerTable>();
            response.Entities = new List<DataExplorerTable>();
            ...
            using (var connection = SqlConnections.NewByKey(request.ConnectionKey))
            {
                ...
                foreach (var t in schemaProvider.GetTableNames(connection))
                {
                    ...
                    response.Entities.Add(new DataExplorerTable
                    {
                        Name = t.Tablename
                    });
                }
            ...
            IEnumerable<DataExplorerTable> result = response.Entities;
        ...
        public ListResponse<PropertyItem> ListColumns(DataExplorerListRequest request)
        {
            request.CheckNotNull();
            Check.NotNullOrEmpty(request.ConnectionKey, "connectionKey");
            ...
            if (!ListTables(new DataExplorerListTablesRequest { ConnectionKey = request.ConnectionKey })
        ...
        public ListResponse<Dictionary<string, object>> List(DataExplorerListRequest request)
        {
            request.CheckNotNull();
            Check.NotNullOrEmpty(request.ConnectionKey, "connectionKey");
            Check.NotNullOrEmpty(request.TableName, "tableName");
